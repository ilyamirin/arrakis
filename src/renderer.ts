...
interface Assets {
  ...
  escapeVictory: HTMLImageElement;
...
export class CanvasRenderer {
  ...
  private drawPreviewTelegraph(
    ...
    const centerX =
      metrics.originX + (previewMove.target.x + 0.5) * metrics.cellSize;
    const centerY =
  ...
  private drawDirectionalSignal(
    ...
    ctx.lineTo(
      tipX - vector.x * cellSize * 0.42 + perpX * cellSize * 0.18,
      tipY - vector.y * cellSize * 0.42 + perpY * cellSize * 0.18,
    );
    ctx.lineTo(
      tipX - vector.x * cellSize * 0.42 - perpX * cellSize * 0.18,
      tipY - vector.y * cellSize * 0.42 - perpY * cellSize * 0.18,
  ...
  private drawEncirclingSignal(centerX: number, centerY: number, cellSize: number): void {
    const { ctx } = this;
    ctx.save();
  ...
  private drawOverlay(state: GameState, metrics: BoardMetrics): void {
    ...
    if (isSinkjawAttack || isVictory) {
      ...
      this.ctx.drawImage(
        isVictory ? this.assets.escapeVictory : this.assets.sinkjawFeast,
        metrics.width / 2 - artSize / 2,
        overlayY + (isVictory ? metrics.cellSize * 0.08 : metrics.cellSize * 0.18),
        artSize,
        artSize,
      );
    }
