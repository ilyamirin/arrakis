...
- Актуальные live-параметры: `8x8`, `20 amber` на поле, цель `20 amber`, `Sinkjaw` radius `4`.
...
- Прямое съедение считается отдельным шансом по ходу:

```text
p(move) = 0, если move <= 3
p(move) = 0.004, если 4 <= move < 32
p(move) = 0.85, если move >= 32
```

- Эта формула откалибрована под `~50%` побед для маршрутного эвристического игрока, который зачищает все `20 amber` на старом поле `8x8`, со средним выигрышным забегом в диапазоне `25-30` ходов.
- По текущему требованию баланс двигать только вероятностью прямого съедения Sinkjaw по ходам; размер поля и `TOTAL_AMBER` не трогать.
