...
- Актуальные live-параметры: `6x6`, `20 amber` на поле, цель `20 amber`, `Sinkjaw` radius `4`.
...
- Эта формула вместе с полем `6x6` откалибрована под `~50%` побед для маршрутного эвристического игрока, который обычно зачищает все `20 amber` за `25-30` ходов.
- Если нужно двигать общий шанс победы дальше, сначала трогай `TOTAL_AMBER`, размер поля или темп цели, а не делай `Sinkjaw` снова более грязным.
