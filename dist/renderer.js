...
export class CanvasRenderer {
    ...
    drawOverlay(state, metrics) {
        ...
        this.roundRect(metrics.originX + metrics.cellSize * 1.1, metrics.originY + metrics.cellSize * 3.2, metrics.boardSize - metrics.cellSize * 2.2, metrics.cellSize * 2.2, metrics.radius * 1.5);
        this.ctx.fill();
        ...
        this.ctx.fillText(state.status === "won" ? "HARVEST COMPLETE" : "SANDWORM STRIKE", metrics.width / 2, metrics.originY + metrics.cellSize * 4.0);
        this.ctx.fillStyle = "rgba(249, 241, 223, 0.86)";
        this.ctx.font = `${metrics.cellSize * 0.22}px "IBM Plex Mono", monospace`;
        this.ctx.fillText(state.status === "won" ? "Запустите New Run для новой раскладки." : "Новая партия доступна по кнопке New Run.", metrics.width / 2, metrics.originY + metrics.cellSize * 4.7);
    }
